...
export const StudentBulkActionBar = ({
  selectedCount = 0,
  onClearSelection,
  onBulkNotify,
  onBulkExport,
  ...
  return (
    ...
    >
      ...
      <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
        <Tooltip title="Send Notification to Selected Students">
          <IconButton size="small" onClick={onBulkNotify} sx={{ color: '#94A3B8', '&:hover': { color: '#FFFFFF' } }}>
            <NotificationsOutlinedIcon sx={{ fontSize: 18 }} />
          </IconButton>
        </Tooltip>

        <Tooltip title="Export Selected Student Directory (CSV)">
          <IconButton size="small" onClick={onBulkExport} sx={{ color: '#94A3B8', '&:hover': { color: '#FFFFFF' } }}>
            <DownloadOutlinedIcon sx={{ fontSize: 18 }} />
          </IconButton>
        </Tooltip>

        <Tooltip title="Activate Selected Memberships">
          <IconButton size="small" onClick={onBulkActivate} sx={{ color: '#22C55E', '&:hover': { color: '#4ADE80' } }}>
            <CheckCircleOutlinedIcon sx={{ fontSize: 18 }} />
          </IconButton>
        </Tooltip>

        <Tooltip title="Deactivate / Block Selected Students">
          <IconButton size="small" onClick={onBulkDeactivate} sx={{ color: '#F59E0B', '&:hover': { color: '#FBBF24' } }}>
...
export default StudentBulkActionBar;
