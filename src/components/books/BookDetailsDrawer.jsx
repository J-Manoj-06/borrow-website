      ...
      <Box sx={{ p: 3, overflowY: 'auto', flexGrow: 1 }}>
        ...
        <Box sx={{ display: 'flex', gap: 1.5, mb: 3, flexWrap: 'wrap' }}>
          ...
          <CustomButton
            variant="outlined"
            startIcon={<PrintIcon />}
            onClick={() => openPrintLabelsModal(copies.length > 0 ? copies : [{ id: `CPY-${book.isbn.replace(/[^0-9X]/gi, '').slice(-6)}-001`, copyId: `CPY-${book.isbn.replace(/[^0-9X]/gi, '').slice(-6)}-001` }], book.title)}
          >
        ...
        {/* Tab 1: Physical Copies Breakdown with QR Code Actions */}
        {activeTab === 1 && (
          <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1.5 }}>
            <Typography variant="caption" sx={{ color: BORROW_COLORS.textSecondary, mb: 1, display: 'block' }}>
              Individual physical copies & scannable QR Code stickers:
            </Typography>

            {(copies.length > 0 ? copies : Array.from({ length: book.totalCopies }).map((_, i) => ({
              ...
              condition: 'Good',
              shelfLocation: book.shelfNumber || 'Shelf-A1',
            }))).map((copy) => (
              <Box
              ...
              >
                <Box>
                  <Typography variant="subtitle2" sx={{ fontWeight: 800, color: BORROW_COLORS.primary, fontFamily: 'monospace' }}>
                    {copy.copyId || copy.id}
                  </Typography>
                  <Typography variant="caption" sx={{ color: BORROW_COLORS.textSecondary }}>
                    Condition: <strong>{copy.condition || 'Good'}</strong> &nbsp;|&nbsp; {copy.shelfLocation || 'CS-01'}
                  </Typography>
                </Box>
                ...
                <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.5 }}>
                  ...
                  <Tooltip title="View Large QR Code">
                    <IconButton size="small" onClick={() => openQrPreview(copy, book.title)} sx={{ color: BORROW_COLORS.primary }}>
                      <QrCode2Icon fontSize="small" />
                    </IconButton>
                  </Tooltip>

                  <Tooltip title="Download QR PNG">
                    <IconButton size="small" onClick={() => downloadSinglePng(copy, book.title)} sx={{ color: BORROW_COLORS.textSecondary }}>
                      <FileDownloadIcon fontSize="small" />
                    </IconButton>
                  </Tooltip>

                  <Tooltip title="View Copy Lifecycle History">
                    <IconButton size="small" onClick={() => openCopyHistory(copy)} sx={{ color: BORROW_COLORS.textSecondary }}>
