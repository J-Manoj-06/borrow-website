...
export const BookImageUploader = ({ currentImageUrl, onFileSelect, onUrlChange, bookId = 'new' }) => {
  ...
  const handleFileChange = async (file) => {
    ...
    // Upload to Cloudinary Storage with progress
    setUploading(true);
    setUploadProgress(0);
    ...
    try {
      ...
      console.error('Cloudinary Storage upload failed:', err);
      setUploading(false);
      ...
      >
        ...
        <Box sx={{ textAlign: { xs: 'center', sm: 'left' }, flexGrow: 1 }}>
          ...
          <Typography variant="body2" sx={{ color: BORROW_COLORS.textSecondary, mb: 2 }}>
            Drag and drop your image file here, or click browse to upload to Cloudinary. Images are auto-compressed to WebP format.
          </Typography>
          ...
          {uploading && (
            <Box sx={{ mb: 2 }}>
              <Typography variant="caption" sx={{ color: BORROW_COLORS.primary, fontWeight: 700, mb: 0.5, display: 'block' }}>
                Uploading to Cloudinary Storage... {uploadProgress}%
              </Typography>
